...
exports.new_patient_scenario = function(patkey, silent, callback) {
...
exports.reviewMasterHealthRecord = function(patientKey, callback) {
    ...
    var start = Date.now();
    d1.getDashboard(patientKey, 'active', function(err, result) {
        if (err) {
            callback(err);
        } else {
            var end = Date.now();
