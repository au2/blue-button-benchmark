...
exports.new_patient_scenario = function(patkey, design, options, callback) {
    var record = dg.generateRecord(options);
    if (! options.silent) {
        console.log('adding patient ' + patkey + '...');
    }
    var start = Date.now();
    design.saveRecord(patkey, record, 'active', function(err, result) {
        if (err) {
            callback(err);
        } else {
            var end = Date.now();
...
exports.reviewMasterHealthRecord = function(patientKey, design, options, callback) {
    var getEntry = function(result, sections, sectionIndex, entryIndex) {
        ...
        var start = Date.now();
        design.getEntry(sectionName, id, function(err) {
            if (err) {
                callback(err);
            } else {
                var end = Date.now();
    ...
    var start = Date.now();
    design.getDashboard(patientKey, 'active', options, function(err, result) {
        if (err) {
            callback(err);
        } else {
            var end = Date.now();
